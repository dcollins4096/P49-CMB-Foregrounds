...
def scrubhtml(url, new_item=False):
    ...
    response = urllib2.urlopen(url_to_use)
    html = response.read()
...
def scrubnew(lines, new_item):
    ...
    for line in lines:
        ...
        if brace_counter == 0 and len(biblines) > 0: #line[0]=='}':
            taking = False
            try:
...
class bibitem():
    ...
    def scrub_journal(self):
        ...
        if self.things.has_key('journal'):
...
#
    ...
    if not options.test and not options.lookup:
        ...
        for newb in newbib.values():
            print ("=========== Adding Title =========== ")
            newb.printer()
            AuthorYearMultiple = 0
            KeepBib = True
            if authors.has_key(newb.FirstAuthor):
                if authors[newb.FirstAuthor].has_key(newb.year):
